...
def receiveOnePing(mySocket, ID, sequence, destAddr, timeout):
    ...
    while 1:
        ...
        type, code, myCheckSum, packetID, sequence = struct.unpack("!bbHHh", header)
        if (type == 0 and packetID == ID):
            toDecimal = struct.calcsize("!d")
            timeOut = struct.unpack("!d", recPacket[28:28 + toDecimal])[0]
            ttl= ord(struct.unpack("!c", recPacket[8:9])[0].decode())
            latency=timeReceived-timeOut
            return (latency, ttl, toDecimal)
        # Fill in end
...
def sendOnePing(mySocket, ID, sequence, destAddr):
    ...
    myChecksum = 0
    # Make a dummy header with a 0 checksum# struct -- Interpret strs as packed binary data
...
def ping(host, timeout=1):
    ...
    for i in range(4):
        delay = doOnePing(dest, ID, i, timeout)
        print("Destination: "+dest+" bytes: "+str(delay[2])+" delay: "+str(int(delay[0]*1000))+"ms Time To Live: "+str(delay[1]))
